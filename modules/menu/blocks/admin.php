...
$menu = mpqn(mpqw($sql = "SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_index WHERE region_id=". (int)(is_numeric($param) ? $param : $param['menu'])." ORDER BY sort"), 'pid', 'id');

if($param['tpl']){ include mpopendir("themes/{$conf['settings']['theme']}/{$param['tpl']}"); return; }

if(strpos($_SERVER['REQUEST_URI'], '/admin') === 0){
	$line = array_shift($m = $menu[0]);
	header("Location: ". $line['href']); 
}
