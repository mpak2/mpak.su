...
if ($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_index"){ //'Отношения'
	stable(
		array(
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
				'uid' => array('*'=>array(0=>'')+spisok("SELECT id, name FROM {$conf['db']['prefix']}users WHERE name<>\"{$conf['settings']['default_usr']}\" ORDER BY name")),
				'usr' => array('*'=>array(0=>'')+spisok("SELECT id, name FROM {$conf['db']['prefix']}users WHERE name<>\"{$conf['settings']['default_usr']}\" ORDER BY name")),
//				'time' => $time,
			),
