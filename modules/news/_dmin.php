...
$conf['settings'] += array(
	...
	"{$arg['modpath']}_index=>title"=>"time,uid,cat_id,img,name",

	"{$arg['modpath']}_cat"=>"Категории",
	"{$arg['modpath']}=>spisok"=>"cat",
);
...
/*	}else if($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_". ($t = "staff")){
	...
	if(!qn("SHOW TABLES LIKE '{$_GET['r']}'")){ # Если таблица не создана, то создаем и переходим к ней
		echo <<<EOF
			<script>
				$(function(){
