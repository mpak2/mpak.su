...
if(empty($conf['settings']['news_post=>title'])) $conf['settings']['news_post=>title'] = "time,uid,kat_id,count,img,name";
...
if(/*true || */$_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_post"){// echo "<div style=float:right;color:#bbb;>{$_GET['r']}:". __LINE__. "</div>";
	stable(
		array(
//			'dbconn' => $conf['db']['conn'],
...
//			'set' => array('orderby'=>$orderby), # Значение которое всегда будет присвоено полю. Исключает любое изменение
			'shablon' => $count_id + array(
				"name" => array("*"=>"<a href=\"/{$arg['modpath']}/{f:id}/{f:{f}}\">{f:{f}}</a>"),
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
				...
				($fn = "kat"). "_id" => array('*'=>spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn}")),
