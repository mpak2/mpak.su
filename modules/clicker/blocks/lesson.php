...
<script>
	$(function(){
		function step(lesson_id){
			$.post("/<?=$arg['modpath']?>:step/null", {lesson_id:lesson_id}, function(data){
				if(isNaN(data)){
					if(data){
//						$("#clicker").show();
//						index = $.parseJSON(data);
//						$.globalEval(index.script);
//						alert(data);
						$.globalEval(data);
					}
				}
...
<? if($arg['access'] > 1): ?>
	<div style="float:right;"><a href="/admin">Админка</a></div>
...
<div id="clicker" style="display:none; position:absolute; left:100px; top:100px;">
	...
	<div class="alert" style="display:none; position:absolute; left:35px; top:0; margin:0px; padding:5px; color:red; font-weight:bold; width:200px; background-color:white; min-height:100px; border:1px solid gray; text-align:left;">текст</div>
	...
	<script>
		$(function(){
			...
			$("#clicker").bind("alert", function(data, param){
				$(this).find(".alert").text("").attr("text", param.alert).css("opacity", 1).show();
				show(param.complete);
			});
