...
$conf['tpl'][$arg['fn']] = mpql(mpqw($sql = "SELECT SQL_CALC_FOUND_ROWS * FROM {$conf['db']['prefix']}{$arg['modpath']}_url WHERE 1=1". ($_GET['check'] ? " AND check=".(int)$_GET['check'] : '').($_GET['id'] ? " AND id=".(int)$_GET['id'] : " ORDER BY id DESC LIMIT ".($_GET['p']*10).",10")));
//echo $sql;
