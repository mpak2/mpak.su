...
class XMPP {
	function eventMessage($fromJid, $content, $offline = FALSE){
		echo "\n$fromJid, $content";
		$xmpp = array_shift(explode("/", $fromJid, 2));
//		$mysql = mysql_query("SELECT t.*, i.uid FROM mp_consultation_index AS i INNER JOIN mp_consultation_chat AS c ON (i.id=c.index_id) LEFT JOIN mp_consultation_chat_txt AS t ON (c.id=t.chat_id) WHERE xmpp=\"". mpquot($xmpp). "\" ORDER BY t.id DESC LIMIT 1");
//		$row = mysql_fetch_array($mysql);

//		mysql_query($sql = "INSERT INTO mp_consultation_chat_txt SET uid=". (int)$row['uid']. ", time=". time(). ", send=". time(). ", chat_id=". (int)$row['chat_id']. ", text=\"". mpquot($content). "\"");

		if($offline){
			$this->sendMessage($fromJid,"Отложенное сообщение принято"); // ответное сообщение
		}else{
			$this->sendMessage($fromJid,"Сообщение принято");  // ответное сообщение
		}
	}
