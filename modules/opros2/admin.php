...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('uid'=>"Пользователь", 'name'=>'Название', 'sort'=>'Сортировка', 'vopros'=>'Вопросов', 'anket'=>"Анкет", 'description'=>'Описание'), # Название полей
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('index_id'=>'Опрос', "type_id"=>"Тип", 'float'=>'Расп', 'name'=>'Вопрос', 'variant'=>'Вариант', 'type'=>'Элемент', 'required'=>'обяз', 'sort'=>'Сорт', 'description'=>'Заголовок'), # Название полей
...
//				'uchastnik' => array('*' => (array)spisok("SELECT id, vopros FROM {$conf['db']['prefix']}{$arg['modpath']}_result", 30)),
				($fn = 'vopros'). '_id' => array('*'=>spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn}")),
				($fn = 'variant'). '_id' => array('*'=>spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn}")),
