...
$spec = mpql(mpqw("SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_index WHERE spec=1"));

?>

                        
                        	<? foreach($spec as $k=>$v): ?>
                        		<div class="SpetialBlock">
                        			<a href="#"><?=$v['name']?></a>
                        			<div class="SpetialImage">
                        				<a href="<?=$arg['modname']?>/<?=$v['id']?>/<?=mpue($v['name'])?>">
                        					<img src="/<?=$arg['modname']?>:img/<?=$v['id']?>/tn:index/w:190/h:120/c:1/null/img.jpg">
                        				</a>
                        			</div>
                        			<div class="SpetialPrice">Цена: <span><?=$v['price']?>р.</span></div> 
                        			<a class="SpetialBuy" href="#">Купить</a>   
                        		</div>
                        	<? endforeach; ?>
