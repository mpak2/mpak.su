...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('type_id'=>'Тип', 'img'=>'Изображение', 'name'=>'Имя', 'price'=>'Цена', 'stock'=>'Акция', 'hit'=>'Хит'), # Название полей
//			'etitle'=> array(),
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
				...
				'stock'=>array('*'=>array('0'=>'', '1'=>'Акция')),
				'hit'=>array('*'=>array('0'=>'', '1'=>'Хит')),
//				'select'=>array('*'=>array('0'=>'Скрыто', '1'=>'Доступно')),
//				'time' => $time,
			),
			'default' => array(
				'uid'=>array('*'=>$conf['user']['uid']),
				'type_id'=>array('*'=>max($_GET['where']['type_id'], $_POST['type_id'])),
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('name'=>'Имя', 'index'=>'Количество', 'img'=>'Фото', 'description'=>'Описание'), # Название полей
//			'etitle' => array('name'=>'Имя', 'img'=>'Фото', 'cnt'=>'Количество', 'description'=>'Описание'), # Название полей
//			'etitle'=> array(),
			'type' => array('img'=>'file', 'description'=>'wysiwyg', 'text'=>'wysiwyg'), # Тип полей
			'ext' => array('img'=>array('image/png'=>'.png', 'image/pjpeg'=>'.jpg', 'image/jpeg'=>'.jpg', 'image/gif'=>'.gif', 'image/bmp'=>'.bmp')),
//			'set' => array('orderby'=>$orderby), # Значение которое всегда будет присвоено полю. Исключает любое изменение
			'shablon' => array(
				'name'=>array('*'=>"<a href=/{$arg['modpath']}/type_id:{f:id}>{f:{f}}</a>"),
...
//				'pcnt'=>array('*'=>"<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_index&where[type_id]={f:id}>Нет</a>")+spisok("SELECT t.id, CONCAT('<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_type&where[type_id]=', t.id, '>', COUNT(*), '_тип</a>') FROM {$conf['db']['prefix']}{$arg['modpath']}_type AS t, {$conf['db']['prefix']}{$arg['modpath']}_type AS t2 WHERE t2.type_id=t.id GROUP BY t.id"),
//				'cnt'=>array('*'=>"<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_index&where[type_id]={f:id}>Нет</a>")+spisok("SELECT t.id, CONCAT('<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_index&where[type_id]=', t.id, '>', COUNT(*), '_вещ</a>') FROM {$conf['db']['prefix']}{$arg['modpath']}_type AS t, {$conf['db']['prefix']}{$arg['modpath']}_index AS i WHERE t.id=i.type_id GROUP BY t.id"),
			), # Шаблон вывода в замене участвуют только поля запроса имеен приоритет перед полем set
//			'disable' => array('orderby'), # Выключенные для записи поля
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
				'type_id' => array('*'=>array(0=>'')+spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_type")),
//				'select'=>array('*'=>array('0'=>'Скрыто', '1'=>'Доступно')),
//				'time' => $time,
			),
