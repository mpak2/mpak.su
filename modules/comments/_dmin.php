...
$conf['settings'] += array(
	"{$arg['modpath']}_deny"=>"Запрет",
	"{$arg['modpath']}_txt"=>"Комментарии",
	"{$arg['modpath']}_url"=>"Адреса",
);
...
if ($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_url"){ //'Ссылки'
	stable(
		array(
//			'dbconn' => $conf['db']['conn'],
...
}elseif($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_txt"){ //'Комментарии'
//mpre(spisok("SELECT id, CONCAT('123 ', name) FROM {$conf['db']['prefix']}{$arg['modpath']}_url"));
	stable(
		array(
//			'dbconn' => $conf['db']['conn'],
			'url' => "/?m[{$arg['modpath']}]=admin&r={$_GET['r']}", # Ссылка для редактирования
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('url_id'=>'Адрес', 'time'=>'Время', 'uid'=>"Пользователь", 'uname'=>'Подпись', 'text'=>'Текст'), # Название полей
			'type' => array('time'=>'timestamp', 'text'=>'textarea'), # Тип полей
//			'ext' => array('img'=>array('image/png'=>'.png', 'image/pjpeg'=>'.jpg', 'image/jpeg'=>'.jpg', 'image/gif'=>'.gif', 'image/bmp'=>'.bmp')),
//			'set' => array('orderby'=>$orderby), # Значение которое всегда будет присвоено полю. Исключает любое изменение
			'shablon' => array(
				'url_id'=>spisok("SELECT t.id, CONCAT('<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_url&where[id]=', u.id, '>', CONVERT(u.name USING utf8), '</a>') FROM {$conf['db']['prefix']}{$arg['modpath']}_txt AS t LEFT JOIN {$conf['db']['prefix']}{$arg['modpath']}_url AS u ON t.url_id=u.id"),
...
//				($fn = 'url'). '_id' => array('*'=>spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn}")),
			),
//			'default' => array('kid'=>$_GET['where']['kid']), # Значение полей по умолчанию
//			'maxsize' => array('bdesc'=>'50', 'sdesc'=>'50'), # Максимальное количество символов в поле
		)
	);
}elseif($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_deny"){ //'Блокировка'
	stable(
		array(
//			'dbconn' => $conf['db']['conn'],
			'url' => "/?m[{$arg['modpath']}]=admin&r={$_GET['r']}", # Ссылка для редактирования
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('url'=>'Ссылка'), # Название полей
//			'type' => array('time'=>'timestamp', 'text'=>'textarea'), # Тип полей
//			'ext' => array('img'=>array('image/png'=>'.png', 'image/pjpeg'=>'.jpg', 'image/jpeg'=>'.jpg', 'image/gif'=>'.gif', 'image/bmp'=>'.bmp')),
//			'set' => array('orderby'=>$orderby), # Значение которое всегда будет присвоено полю. Исключает любое изменение
...
//			'spisok' => array( # Список для отображения и редактирования
//				'admin' => array('*'=>spisok("SELECT id, name FROM {$GLOBALS['conf']['db']['prefix']}admin")),
//				'uid' => array('*'=>spisok("SELECT id, url FROM {$GLOBALS['conf']['db']['prefix']}{$arg['modpath']}_url")),
//			),
//			'default' => array('kid'=>$_GET['where']['kid']), # Значение полей по умолчанию
//			'maxsize' => array('bdesc'=>'50', 'sdesc'=>'50'), # Максимальное количество символов в поле
		)
