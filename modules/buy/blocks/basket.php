...
$orders = mpql(mpqw("SELECT SUM(o.count) AS cnt FROM {$conf['db']['prefix']}{$arg['modpath']}_basket AS b LEFT JOIN {$conf['db']['prefix']}{$arg['modpath']}_basket_orders AS o ON (b.id=o.basket_id) WHERE b.status=0 AND b.uid=". (int)$conf['user']['uid']. " GROUP BY b.id"), 0);// mpre($dat);

//mpre($orders);

?>
<div class="ShopCart">
	<a href="/<?=$arg['modname']?>:basket">В вашей корзине:</a><br>
	<span class="basket_counter"><?=(int)$orders['cnt']?></span> товаров
</div>