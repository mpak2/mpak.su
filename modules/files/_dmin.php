...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			array("w"=>"Ширина", "h"=>"Высота", "c"=>"Обрезание", "activ"=>"Активность"),
			'type' => array('time'=>'timestamp', 'up'=>'timestamp', "name"=>($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_files" ? 'file' : ""), "file"=>"file", 'period'=>"timecount", 'sort'=>'sort', 'img'=>'file', 'img2'=>'file', 'duration'=>'timecount', 'description'=>'textarea', 'text'=>'wysiwyg'), # Тип полей
			'ext' => array(
				'name'=>array('*'=>'*'),
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => $spisok += array( # Список для отображения и редактирования
				...
				($fn = "cat") => array('*'=>array("")+spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn}")),
