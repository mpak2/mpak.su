...
}elseif($id = $_POST['text']['text_id']){
	echo mpql(mpqw("SELECT text FROM {$conf['db']['prefix']}{$arg['modpath']}_text WHERE id=". (int)$id), 0, 'text');
	exit;
}elseif($arg['access'] >= 5){
//	echo "test ". mpsettings("subscribe_send_{$arg['fn']}_test", "Бла бла бла");
	...
	if(!$_GET['to']){
		header("Location: ". $_SERVER['REQUEST_URI']. "/to:100000");
		exit;
	}
	$conf['tpl']['text'] = mpqn(mpqw("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_text"));
