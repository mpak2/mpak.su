...
$conf['tpl']['vacancy'] = mpql(mpqw($sql = "SELECT SQL_CALC_FOUND_ROWS id.*, u.name AS uname
	FROM ". ($tn = "{$conf['db']['prefix']}{$arg['modpath']}_index"). " AS id
	LEFT JOIN {$conf['db']['prefix']}users AS u ON id.uid=u.id
	WHERE 1". ($_GET['cat_id'] ? " AND id.cat_id=". (int)$_GET['cat_id'] : ""). ($_GET['id'] ? " AND id.id=". (int)$_GET['id'] : ""). "
	LIMIT ".($_GET['p']*10).", 10"
));
