...
$list = mpql(mpqw("SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_type LIMIT 30"));
// $min = $max = $list[0]['read'];
// foreach($list as $k=>$v){
// 	$max = max($max, $v['read']);
// 	$min = min($min, $v['read']);
...
//	$tagcloud .= "<a href=\"/{$arg['modpath']}:cat/{$v['id']}\" rel=\"tag\" style=\"font-size: ". (4 + (int)(20 / $max * ($v['read']+0.001))). "px;\">". $v['name']. "</a>";
	$tagcloud .= "<a href=\"/{$arg['modpath']}:type//type_id:{$v['id']}\" rel=\"tag\" style=\"font-size: 10px;\">". $v['name']. "</a>";
}
