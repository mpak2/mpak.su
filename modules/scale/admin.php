...
mpmenu($menu = array('Утверждение', 'Аргументы'));

if ($menu[(int)$_GET['r']] == 'Утверждение'){
	stable(
		array(
			'dbconn' => $conf['db']['conn'],
			'url' => "?m[{$arg['modpath']}]=admin&r={$_GET['r']}", # Ссылка для редактирования
			'name' => "{$conf['db']['prefix']}{$arg['modpath']}_qw", # Имя таблицы базы данных
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('name'=>'Утверждение', 'src'=>'Источник', 'hide'=>'Видим', 'yes'=>'Согласных', 'no'=>'Несогласных', 'description'=>'Описание'), # Название полей
			'type' => array('name'=>'textarea', 'description'=>'textarea'), # Тип полей
//			'ext' => array('img'=>array('image/png'=>'.png', 'image/pjpeg'=>'.jpg', 'image/jpeg'=>'.jpg', 'image/gif'=>'.gif', 'image/bmp'=>'.bmp')),
//			'set' => array('orderby'=>$orderby), # Значение которое всегда будет присвоено полю. Исключает любое изменение
			'shablon' => array(
				'name'=>array('*'=>'<a href="/?m[scale]=admin&r=1&where[qwid]={f:id}">{f:{f}}</a>'),
				'src'=>array('*'=>'<a target=_blank href="{f:{f}}">{f:{f}}</a>'),
				'yes'=>spisok("SELECT qw.id, COUNT(*) FROM {$conf['db']['prefix']}{$arg['modpath']}_qw as qw, {$conf['db']['prefix']}{$arg['modpath']}_mess AS mess WHERE qw.id=mess.qwid AND yes=1 GROUP BY qw.id"),
				'no'=>spisok("SELECT qw.id, COUNT(*) FROM {$conf['db']['prefix']}{$arg['modpath']}_qw as qw, {$conf['db']['prefix']}{$arg['modpath']}_mess AS mess WHERE qw.id=mess.qwid AND yes=0 GROUP BY qw.id"),
...
}elseif ($menu[(int)$_GET['r']] == 'Аргументы'){
	stable(
		array(
			'dbconn' => $conf['db']['conn'],
			'url' => "?m[{$arg['modpath']}]=admin&r={$_GET['r']}", # Ссылка для редактирования
			'name' => "{$conf['db']['prefix']}{$arg['modpath']}_mess", # Имя таблицы базы данных
//			'where' => '', # Условия отбора содержимого
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('qwid'=>'Утверждение', 'time'=>'Время', 'yes'=>'Позиция', 'name'=>'Имя', 'description'=>'Описание'), # Название полей
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
				'qwid' => array('*'=>spisok("SELECT id, name FROM {$GLOBALS['conf']['db']['prefix']}{$arg['modpath']}_qw")),
				'yes' => array('*'=>array('0'=>'Несогласен', "1"=>'Согласен')),
			),
			'default' => array('qwid'=>max($_GET['where']['qwid'], $_POST['qwid'])), # Значение полей по умолчанию
//			'maxsize' => array('bdesc'=>'50', 'sdesc'=>'50'), # Максимальное количество символов в поле
		)
	);
}
