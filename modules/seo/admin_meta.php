...
if(!$canonical){ mpre("Каноническая ссылка не установлена"); # Нет мета или обновление категории больше чем у записи
		...
		if(!$seo_href = "/seo:admin/r:{$conf['db']['prefix']}seo_cat?&where[id]={$seo_cat['id']}"){ mpre("ОШИБКА формирования адреса перехода на СЕО категорию");
			...
			"В адресе категории <a target=blank href='{$seo_href}'>{$seo_cat['name']}</a> и метаинформации заменены не все теги", $href, $meta,
			"Свойства страницы", $ZAM,
			"Связанные таблицы", $ZAM_INDEX,
			"Параметры адресной строки", $ZAM_GET
		);}
