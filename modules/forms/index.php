...
if($_GET['id']){
	...
	if($anket = $conf['tpl']['index'][ $_GET['id'] ]){
			...
			foreach($t as $v){
				if(!empty($v['tn'])){
					...
					if(array_search($v['type'], array("radio", "select")) !== false){
						...
						$conf['tpl']['variant'][ $v['id'] ] = mpqn(mpqw("SELECT * FROM ". mpquot($v['tn']). " WHERE 1 ". ($fields['uid'] ? " AND uid=". (int)$conf['user']['uid'] : ""). " AND ". mpquot($key). "=0 ORDER BY id"));
