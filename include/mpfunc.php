...
//			$event_log = mpql(mpqw("SELECT LAST_INSERT_ID() AS cnt FROM {$conf['db']['prefix']}users_event_log"), 0, 'cnt'); //$event_log = mysql_insert_id();
			$event_log = mpql(mpqw("select max(last_insert_id(id)) AS cnt FROM mp_users_event_log WHERE event_id=". (int)$event['id']), 0, 'cnt');
