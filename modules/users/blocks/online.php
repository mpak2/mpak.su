...
if(!$last_time = (time()-abs($conf['settings']['sess_time']))){ mpre("ОШИБКА расчета времени сессии");
}elseif(!$SESS = qn($sql = "SELECT * FROM {$conf['db']['prefix']}users_sess WHERE last_time > ". $last_time. " AND LENGTH(sess)=32 ORDER BY last_time DESC")){
}elseif(!$USERS = rb("{$conf['db']['prefix']}users", "id", "id", rb($SESS, "uid"))){ mpre("ОШИБКА Пользователи не установлены");
	...
	<? foreach($USERS as $uid=>$users): ?>
		<? if(!$uid):// mpre("Гость") ?>
		<? elseif(!$user = rb("users-", "id", $uid)): mpre("Ошибка определения пользователя") ?>
		<? else:// mpre("users", $users) ?>
			<div style="display:inline-block; margin:1px; border:1px solid #ddd; position:relative;" title="<?=$user['name']?>">
				<a href="/users/412707">
					...
					<img src="/users:img/<?=$uid?>/tn:/w:40/h:40/c:1/null/img.jpg" style="z-index:-1; padding:1px;">
				</a>
			</div>
		<? endif; ?>
	<? endforeach; ?>
		...
		<? else: ?>
			<div style="display:inline-block; margin:1px; border:1px solid #ddd; position:relative;" title="<?=$os?>">
				<a href="/users/412707">
					...
					<img src="/users:img/0/tn:/w:40/h:40/c:1/null/img.jpg" style="z-index:-1; padding:1px;">
				</a>
			</div>
		<? endif; ?>
	<? endforeach; ?>
