...
$vendor = mpqn(mpqw("SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_vendor"), 'name');
$type = mpqn(mpqw("SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_type ORDER BY sort"), 'name');
...
if(array_key_exists('blocks', $_GET['m']) && array_key_exists('null', $_GET) && ($_GET['id'] == $arg['blocknum']) && $_FILES){
		if($_POST['price_id'] || !empty($_FILES['csv']['tmp_name'])){
			if($_POST['price_id']){
				...
				if($_POST['price_id'] && ($fn = mpfn("{$conf['db']['prefix']}{$arg['modpath']}_price", "csv", $_POST['price_id'], null, array('text/csv'=>'.csv', 'text/comma-separated-values'=>'.csv')))){
			...
			while (($data = fgetcsv($f, 0, ",", "\"")) !== false){
				if($tmp_line++ == 0){
					$fn = array("Ширина"=>"width", "Имя"=>"name", "Количество"=>"count", "Цена"=>"price", "Накрутка"=>"tamper", "Доставка"=>"delivery", "Стоимость_1"=>"coast_1", "Стоимость_2"=>"coast_2");
				}
				$min = min(count($fn), count($data));
				...
				if($v['name'] = trim($v['name'])){
					preg_match($r = "/([0-9]+)\/([0-9]+)([R\/][0-9]+)(C|LT)?(.*)/", $v['name'], $reg);
					if($reg[3]){
						$v['diameter_id'] = mpfdk("{$conf['db']['prefix']}{$arg['modpath']}_diameter",
							array("name"=>strtr($reg[3], array('/'=>'R')). $reg[4]),
							array("name"=>strtr($reg[3], array('/'=>'R')). $reg[4])
						);
					}// mpre($reg);

					if($reg[1] && $reg[2] && $reg[3]){
						$v['premium_id'] = mpfdk("{$conf['db']['prefix']}{$arg['modpath']}_premium",
							array("name"=>"{$reg[1]}/{$reg[2]}". strtr($reg[3], array('/'=>'R')). $reg[4]),
							array("name"=>"{$reg[1]}/{$reg[2]}". strtr($reg[3], array('/'=>'R')). $reg[4])
						);
					}
					...
					mpfdk("{$conf['db']['prefix']}{$arg['modpath']}_index", array('name'=>$v['name']), $v+array('premium'=>1), $v);
				}
