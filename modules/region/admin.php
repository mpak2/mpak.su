...
$conf['settings'] += array(
	"{$arg['modpath']}_index"=>"Регионы",
	"{$arg['modpath']}_href"=>"Ссылки",
);
...
//			'etitle'=> array(),
			'type' => array('img'=>'file', 'sort'=>'sort', 'description'=>'wysiwyg', 'text'=>'wysiwyg'), # Тип полей
...
//				($f = 'type_id')=>array('*'=>max($_GET['where'][$f], $_POST[$f])),
			...
			'maxsize' => array('description'=>300,), # Максимальное количество символов в поле
		)
	);
}elseif($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_href"){
	stable(
		array(
//			'dbconn' => $conf['db']['conn'],
			'url' => "/?m[{$arg['modpath']}]=admin&r={$_GET['r']}", # Ссылка для редактирования
