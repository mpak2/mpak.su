...
if(!is_array($SEO_ROBOTS = call_user_func(function() use($conf, $arg){
...
}elseif(!is_array($themes_index = get($conf, 'themes', 'index'))){ mpre("Определение хоста");
}elseif(get($themes_index, 'hide') && (print("User-agent: *\n\nDisallow: /"))){ mpre("Сайт скрыт, так как является <a href=''>зеркалом</a>");
}else{
	foreach(rb($SEO_ROBOTS_AGENT, "name") + ['*'=>['name'=>'*'], 'Yandex'=>['name'=>'Yandex']] as $seo_robots_agent){// mpre($seo_robots_agent);
		...
		}elseif(!is_array($seo_robots = rb($SEO_ROBOTS, 'themes_index', get($themes_index, 'id')))){ mpre("Ошибка определения робота", $SEO_ROBOTS, $seo_robots, get($conf, 'themes', 'index', 'id'));
