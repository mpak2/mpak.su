...
if(($arg['access'] > 1) && array_key_exists('blocks', $_GET['m']) && array_key_exists('null', $_GET) && ($_GET['id'] == $arg['blocknum']) && $_POST){
	...
	$data = mpql(mpqw("SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_{$arg['fn']} WHERE uid=". (int)$conf['user']['uid']. " ORDER BY time DESC"));
}
...
<? if(empty($_POST)): ?>
	...
	<div class="add_<?=$arg['blocknum']?>" style="text-align:right; margin-bottom:10px; display:none;">
