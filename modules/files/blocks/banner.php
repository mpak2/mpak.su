...
$file = mpql(mpqw("SELECT * FROM {$conf['db']['prefix']}{$arg['modpath']}_files WHERE 1 ". ($param['Категория'] ? " AND cat_id=".(int)$param['Категория'] : ""). " ORDER BY RAND() LIMIT 1"), 0);

?>
<? if($file['href']): ?><a href="<?=$file['href']?>"><? endif; ?>
	<img src="/files/<?=$file['id']?>/w:<?=($param['Ширина'] ?: 200)?>/h:<?=($param['Высота'] ?: 200)?>/c:1/null/img.jpg">
<? if($file['href']): ?></a><? endif; ?>