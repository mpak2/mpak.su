...
if($_GET['id']){
	...
	foreach($conf['tpl']['vopros'] as $t){
		foreach($t as $v){
			if(!empty($v['tn']) && (array_search($v['type'], array("radio", "select")) !== false)){
	...
	if($_POST && ($anket = $conf['tpl']['index'][ $_GET['id'] ])){
		...
		if($conf['tpl']['anket_id'] = mysql_insert_id()){
			...
			} mpevent("Заполнение формы", $_SERVER['REQUERT_URI'], $conf['tpl']['index'][ $_GET['id'] ]['uid'], "/{$arg['modpath']}/anket_id:{$conf['tpl']['anket_id']}", implode("<br />", $res), $_POST);
			if($fds) $mysql_inset_id = mpfdk($anket['tn'], null, array("time"=>time(), "uid"=>$conf['user']['uid'])+(array)$fds);
		}
	}else{
		mpevent("Просмотр формы", $_SERVER['REQUERT_URI'], $conf['tpl']['index'][ $_GET['id'] ]['uid']);
	}
}else{
