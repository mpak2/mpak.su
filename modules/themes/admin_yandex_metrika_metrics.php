...
if(($n = date("N")) && ($date1 = date("Y-m-d", strtotime((-$n/*-6*/+1+($offset = get($_GET, "week")*7)). " days"))) && ($date2 = date("Y-m-d", strtotime((-$n + 7 + $offset). " days")))){
	if($tpl['yandex_metrika_period'] = fk("yandex_metrika_period", $w = array("date1"=>$date1, "date2"=>$date2), $w)){
		if(get($_POST, "yandex_metrika_id")){
			if($yandex_metrika_period = rb("yandex_metrika_period", "id", $_POST['yandex_metrika_period_id'])){
				if($yandex_metrika = rb("yandex_metrika", "id", $_POST["yandex_metrika_id"])){
					if($yandex_token = rb("yandex_token", "id", rb("yandex", "yandex_token_id"))){
						...
						if($data = file_get_contents("https://api-metrika.yandex.ru/stat/v1/data?preset=sources_summary&metrics=". implode(",", $metrics). "&date1={$yandex_metrika_period['date1']}&date2={$yandex_metrika_period['date2']}&id={$yandex_metrika['id']}&oauth_token={$yandex_token['name']}")){
							if($json = json_decode($data, true)){// exit(mpre($json));
								foreach($json['data'] as $d){// (mpre($d['metrics']));
									if($yandex_metrika_dimensions = fk("yandex_metrika_dimensions", $w = array("name"=>$d['dimensions'][0]['name'], "alias"=>$d['dimensions'][0]['id']), $w)){
										$yandex_metrika_metrics = fk("yandex_metrika_metrics",
