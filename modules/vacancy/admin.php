...
if ($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_index"){ //'Вакансии'
	stable(
		array(
			'dbconn' => $conf['db']['conn'],
...
//			'title' => array('profa'=>'Профессия', 'wage'=>'Заработная плата', 'sort'=>'Сортировка', 'description'=>'Описание'), # Название полей
//			'etitle'=> array(),
			'type' => array('time'=>'timestamp', 'sort'=>'sort', 'description'=>'textarea'), # Тип полей
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
				...
				($fn = 'cat'). '_id' => array('*'=>spisok("SELECT c3.id, CONCAT(c1.name, ' # ', c2.name, ' # ', c3.name) FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn} AS c1 INNER JOIN {$conf['db']['prefix']}{$arg['modpath']}_{$fn} AS c2 ON c1.id=c2.cat_id INNER JOIN {$conf['db']['prefix']}{$arg['modpath']}_{$fn} AS c3 ON c2.id=c3.cat_id")),
...
}elseif ($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_cat"){// echo "{$_GET['r']}:". __LINE__;
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('cat_id'=>'Родитель', 'name'=>'Имя', 'cat'=>'Категорий', 'description'=>'Описание'), # Название полей
//			'etitle'=> array(),
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => array( # Список для отображения и редактирования
//				'uid' => array('*'=>spisok("SELECT id, name FROM {$conf['db']['prefix']}users")),
				($fn = 'cat'). '_id' => array('*'=>array(0=>'')+spisok("SELECT id, name FROM {$conf['db']['prefix']}{$arg['modpath']}_{$fn} ORDER BY name")),
