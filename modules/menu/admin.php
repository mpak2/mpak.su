...
//			'dbconn' => $conf['db']['conn'],
			...
			'where' => (empty($_GET['where'][ array_shift(explode("=", $conf['settings']["{$t}=>where_empty"])) ]) && $conf['settings']["{$t}=>where_empty"] ? $conf['settings']["{$t}=>where_empty"] : $conf['settings']["{$t}=>where"]), # Условия отбора содержимого
...
//				(($fn = 'index'). ($prx = ''))=>array('*'=>"<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}&where[index_id]={f:id}&where[region_id]={f:region_id}>Нет</a>")+spisok("SELECT r.id, CONCAT('<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}&where[index_id]=', r.id, '&where[region_id]=', r.region_id, '>', COUNT(*), '_пункт</a>') FROM {$_GET['r']} AS r, {$conf['db']['prefix']}{$arg['modpath']}_index AS fn WHERE r.id=fn.index_id GROUP BY r.id"),
...
//			'hidden' => array('name', 'enabled'), # Скрытые поля
			'spisok' => $spisok += array( # Список для отображения и редактирования
				...
				"modules"=>array("*"=>array("")+spisok("SELECT id,name FROM {$conf['db']['prefix']}modules")),
//				($fn = "modules")=>array("*"=>array("")+spisok("SELECT id,name FROM {$conf['db']['prefix']}modules")),
