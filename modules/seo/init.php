...
if("sqlite" == $conf["db"]["type"]){
	if(mpsettings($t = "seo_index_themes", "Адресация") && ($table = ("{$conf['db']['prefix']}{$t}"))){
		qw("CREATE TABLE `{$table}`(`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE,
		`time` INTEGER,
		`uid` INTEGER,`index_id` INTEGER,
		`location_id` INTEGER,
		`themes-index` INTEGER
		)");
...
}else{ mpre("Тип данных базы данных не установлен", $conf["db"]["type"]); }