...
$conf['tpl']['all'] = mpql(mpqw("SELECT c.*, COUNT(*) as count FROM {$conf['db']['prefix']}{$arg['modpath']}_cat as c, {$conf['db']['prefix']}{$arg['modpath']}_index as p WHERE c.id=p.cat_id AND parent=".(int)$_GET['cid']." GROUP BY c.id"));
