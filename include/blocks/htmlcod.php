...
if ((int)$arg['confnum']){
	...
	if (isset($_POST[ $arg['confnum'] ]['text'])){
		...
		$param = mpql(mpqw("SELECT param FROM {$GLOBALS['conf']['db']['prefix']}blocks WHERE id = {$arg['confnum']}"), 0, 'param');
	}
...
echo mpql(mpqw("SELECT param FROM {$GLOBALS['conf']['db']['prefix']}blocks WHERE id = {$arg['blocknum']}"), 0, 'param');
