...
if(get($_POST,'f')){
...
}else if(get($_POST,'text') && array_key_exists('null', $_GET)){
	...
	exit;
}else if(get($_POST,'submit') && !empty(get($_POST,'plan'))){
	$max = mpql(mpqw("SELECT MAX(sort)+1 AS max FROM {$conf['db']['prefix']}{$arg['modpath']}_plan"), 0, 'max');
	$sql = "INSERT INTO {$conf['db']['prefix']}{$arg['modpath']}_plan SET uid=".(int)$conf['user']['uid'].", time=".time().", plan=\"".($arg['admin_access'] >= 4 ? mpquot($_POST['plan']) : htmlspecialchars(mpquot($_POST['plan'])))."\", performers_id=". (int)$_POST['performers_id']. ", sort=".(int)$max;
	$tesult = fk(
		"{$conf['db']['prefix']}{$arg['modpath']}_plan",
		null,
		[
			"uid"=>(int)$conf['user']['uid'],
			"time"=>time(),
			"plan"=>$arg['admin_access'] >= 4 ? mpquot($_POST['plan']) : htmlspecialchars(mpquot($_POST['plan'])),
			"performers_id"=>(int)$_POST['performers_id'],
			"sort"=>intval($max)
		]
	);
	
	mpevent("Постановка новой задачи", "http://". mpidn($_SERVER['HTTP_HOST']). "/{$arg['modname']}/". $tesult['id'], $conf['user']['uid']);
	header("Location: ". $_SERVER['REQUEST_URI']);
}elseif(get($_POST,'golos')){
...
/*elseif(($arg['admin_access'] >= 5) && array_key_exists('value', $_POST) && $_POST['id']){
...
$tpl['cc'] = mpqn(mpqw("SELECT p.*, p.id AS plan_id, p.cat_id, COUNT(*) AS cnt FROM {$conf['db']['prefix']}{$arg['modpath']}_plan AS p LEFT JOIN {$conf['db']['prefix']}{$arg['modpath']}_cat AS c  ON (c.id=p.cat_id) WHERE 1". (get($_GET,'performers_id') ? " AND performers_id=". intval($_GET['performers_id']) : ""). " GROUP BY c.id"), 'cat_id');

$tpl['dev'] = mpqn(mpqw($sql = "SELECT SQL_CALC_FOUND_ROWS * FROM ". ($tn = "{$conf['db']['prefix']}{$arg['modpath']}_plan"). mpwr($tn)." ORDER BY time DESC LIMIT ".(get($_GET,'p')*5).",5"));// echo $sql;
