...
(function($){
	...
	Redactor.prototype = {
		...
		resizeImage: function(resize)
		{
			...
			{			
				// Изменение изображения в зависимости от его свойство
				src = $(resize).attr("src");// alert(src);
				width = $(resize).css("width");// alert(width);
				height = $(resize).css("height");// alert(height);
				src = src.replace(new RegExp('w:([0-9]+)[^\/]', 'ig'),"w:"+parseInt(width));
				src = src.replace(new RegExp('h:([0-9]+)[^\/]', 'ig'),"h:"+parseInt(height));
				$(resize).attr("src", src);
