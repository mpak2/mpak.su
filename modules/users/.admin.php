...
if($_GET['r'] == "{$conf['db']['prefix']}{$arg['modpath']}_event_send"){
	stable(
		array(
//			'dbconn' => $conf['db']['conn'],
...
//			'set' => array('name'=>'kanal'), # Значение которое всегда будет присвоено полю. Исключает любое изменение
			'shablon' =>array(
				'count'=>array('*'=>'<a href=/?m[users]=admin&r=2&where[gid]={f:id}>Нет</a>')+spisok("SELECT g.id, CONCAT('<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_mem&where[gid]=', g.id, '>', COUNT(*), ' участник', '</a>') FROM {$conf['db']['prefix']}{$arg['modpath']}_grp AS g, {$conf['db']['prefix']}{$arg['modpath']}_mem AS m WHERE g.id=m.gid GROUP BY g.id"),
...
//			'bottom' => array('tr'=>'<tr>', 'td'=>"<td valign='top'>", 'shablon'=>'<tr><td>{config:url}</td></tr>'), # Формат записей таблицы
			...
			'title' => array('time'=>'Время', 'event_id'=>"Событие", 'uid'=>"Пользователь", "own"=>"Владелец", 'return'=>'Функция', 'description'=>'Описание'), # Название полей
//			'etitle'=> array(),
			'type' => array('time'=>'timestamp', 'description'=>"textarea", 'zam'=>'textarea', 'text'=>'wysiwyg'), # Тип полей
//			'ext' => array('img'=>array('image/png'=>'.png', 'image/pjpeg'=>'.jpg', 'image/jpeg'=>'.jpg', 'image/gif'=>'.gif', 'image/bmp'=>'.bmp')),
//			'set' => array('orderby'=>$orderby), # Значение которое всегда будет присвоено полю. Исключает любое изменение
//			'shablon' => array(
//				'name'=>array('*'=>"<a href=/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_index&where[rid]={f:id}>{f:{f}}</a>"),
