...
//		xml_parser_get_option ($this->_parser, XML_OPTION_SKIP_TAGSTART, 1);
...
class IXR_Server {
    ...
    function serve($data = false) {
        ...
        if (!$this->message->parse()) {
            $this->error(-32700, 'parse error. not well formed1');
        }
...
class IXR_Client {
    ...
    function query() {
        ...
        if (!$this->message->parse()) {
            // XML error
            $this->error = new IXR_Error(-32700, 'parse error. not well formed2');
            return false;
        }
