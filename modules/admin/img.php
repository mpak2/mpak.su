...
if(get($_GET, 'tn')){
	...
	}else if(!$img = mprs($file_name, (get($_GET, "w") ?: 0), (get($_GET, "h") ?: 0), (get($_GET, "c") ?: 0))){ mpre("ОШИБКА формирования изображения");
	}else{ header("Content-type: image/{$exp}");
		echo $img;
	}
}elseif(array_key_exists('', $_GET) && ($file_name = mpopendir("modules/{$arg['modpath']}/img/". basename($_GET[''])))){
	if(!$img = mprs($file_name, (get($_GET, "w") ?: 0), (get($_GET, "h") ?: 0), (get($_GET, "c") ?: 0))){ mpre("ОШИБКА формирования статисческого изображения");
