...
}else if(!$sess = users_sess()){ pre("ОШИБКА создания сесии");
}else if(!$sess = fk("users-sess", ["id"=>$sess["id"]], null, ["uid"=>$users["id"]])){ pre("ОШИБКА правки текущей сессии пользователя");
}else{ //pre($sess);
