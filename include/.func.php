		...
		if ( isset($table['shablon']) && !$zamena_sql = array()){
			foreach($line as $sk=>$sv){
				if ($table['type'][$sk] == 'timestamp'){
					$zamena_sql["{f:$sk}"] = strtr(date('Y.m.d H:i:s', $sv), array(' '=>'&nbsp;'));
			...
			foreach($table['title'] ? $table['title'] : $table['_fields'] as $k=>$v){
				...
				if (!isset($hidden[$k])){
					...
					if (isset($table['shablon'][$k]['*']) || isset($table['shablon'][$k][ $line['id'] ])){ # Если есть шаблон для поля в списке
						...
						echo $string;
					}elseif(gettype($table['spisok'][$k]['*'][$v]) == 'string'){ # Задан списоk на все поле
//						echo "<span style=color:blue;>". $table['spisok'][$k]['*'][$v]. "</span>";
					}elseif(gettype($table['spisok'][$k][$id][$v]) == 'string'){# Задан список для поля и id
//						echo "<span style=color:blue;>". $table['spisok'][$k][$id][$v]. "</span>";
						...
						if ($table['type'][$k] == 'file'){
							...
							echo is_numeric($v) ? "<div title='$v'>".str_replace(' ', '&nbsp;', date('Y.m.d H:i:s', $v))."</div>" : htmlspecialchars($v);
						}elseif($table['type'][$k] == 'sort'){
							echo "<nobr><a href='{$table['url']}&p={$_GET['p']}".($table['order'] ? "&order={$_GET['order']}" : '')."&dec[$k]={$line['id']}' class='".(strpos($table['order'], 'DESC') || strpos($table['order'], 'desc') ? 'down' : 'up')."'><img src='/img/n.gif'></a><span>$v</span><a href='{$table['url']}&p={$_GET['p']}".($table['order'] ? "&order={$_GET['order']}" : '')."&inc[$k]={$line['id']}' class='".(!strpos($table['order'], 'DESC') && !strpos($table['order'], 'desc') ? 'down' : 'up')."'><img src=/img/n.gif></a></nobr>";
							...
							if($name = mpql(mpqw("SELECT name FROM {$conf['db']['prefix']}{$arg['modpath']}_". ($f = substr($k, 0, strlen($k)-3)). " WHERE id=". (int)$v), 0, 'name')){
								echo "<a href=\"/?m[{$arg['modpath']}]=admin&r={$conf['db']['prefix']}{$arg['modpath']}_{$f}&where[id]={$v}\">{$name}</a>";
							}else{
								echo $v;
							}
						}else{
...
//						}
						...
						if ($table['type'][$k] == 'textarea' || ($table['type'][$k] == 'wysiwyg' && !$edit)){
							echo "<textarea name='$k' style='width:100%; height:100px;'".($disable[$k] || $table['set'][$k] ? ' disabled' : '').">".($_GET['edit'] || $_GET['cp'] ? htmlspecialchars($edit[$k]) : htmlspecialchars($table['set'][$k]))."</textarea>";
							...
							echo "<input type='text' name='$k'".($_GET['edit'] || $_GET['cp'] ? " value='".strtr(htmlspecialchars($edit[$k]), array('\''=>'&#39;'))."'" : " value='".@htmlspecialchars(strtr($table['default'][$k], array('\''=>'&#39;')))."'")." style='width:100%'".($disable[$k] || $table['set'][$k] || empty($table['_fields'][$k]) ? ' disabled' : '').">";
						}
					}
